...
class AudioManager:
    ...
    def __init__(self, audio_config_path='/etc/sculpture/audio.conf'):
    ...
    def _load_audio_config(self):
        ...
        try:
            if Path(self.audio_config_path).exists():
                config.read(self.audio_config_path)
                logger.info(f"Loaded audio configuration from {self.audio_config_path}")
            else:
                logger.warning(f"Audio configuration file not found at {self.audio_config_path}. Using default values.")
        except Exception as e:
