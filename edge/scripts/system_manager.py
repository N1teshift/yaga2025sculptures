...
class SystemManager:
    ...
    def switch_to_local_mode(self, track=None, audio_config=None):
        """Switch to local mode - playback from local files."""
        try:
    ...
    def get_available_tracks(self):
        ...
        try:
            ...
            if not loops_dir.is_dir():
                ...
                tracks = sorted([f.name for f in loops_dir.iterdir() 
    ...
    def update_loop_track(self, track_path, audio_config=None):
        ...
        samplerate = audio_config.get('samplerate', '22050')
        device = audio_config.get('device', 'pulse/sculpture_sink')
        audio_format = audio_config.get('format', 's16')
